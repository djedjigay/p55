...
fetch("http://localhost:3000/api/products/"+id)
  .then((res) => res.json())
  .then((objetProduit) => {
    // execution de la fontion formatProduit
    formatProduit(objetProduit);
  })
  .catch((err) => {
...
function formatProduit(produit) {
  ...
  let couleurOption = document.querySelector("select");

  const image = document.createElement('img');
  imageAlt.appendChild(image);
  ...
  formatColors(couleurOption, produit.colors);
